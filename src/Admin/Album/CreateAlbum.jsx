...
const CreateAlbum = () => {
  ...
  let handleSubmit = async (e) => {
    ...
    try {
      ...
      if (albumThumbnail) {
        ...
        albumData.append("cloud_name", "dmqeabn0d");

        let response = await fetch(
          "https://api.cloudinary.com/v1_1/dmqeabn0d/upload",
          {
            method: "POST",
            body: albumData,
          }
      ...
      await Promise.all(
        songDetails.map(async (song, index) => {
          //! add the song thumbnail and song file (.mp3)
          ...
          if (song.songThumbnail) {
            ...
            songFileData.append("cloud_name", "dmqeabn0d");
            //^ upload song thumbnail to cloudinary using fetch api
            let response = await fetch(
              "https://api.cloudinary.com/v1_1/dmqeabn0d/upload",
              {
                method: "POST",
          ...
          if (song.songFile) {
            ...
            songDataObject = {
              ...
              size:
                uploadedSongFileResult.bytes / (1024 * 1024).toFixed(2) + "MB",
            };
          }
  ...
  return (
    <section className="w-full flex flex-col justify-center items-center p-6 text-white overflow-y-scroll">
